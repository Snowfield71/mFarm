...
> 本文以当前仓库代码为准。项目主体是 Cocos Creator 3.8.x TypeScript 客户端；`Server` 目录包含 Rust/Axum + MySQL 服务端和静态美术资源，但客户端业务数据接口尚未正式接入，不应把服务端路由描述为已经上线的完整闭环。
...
- 语言：TypeScript。
- 设计分辨率：`360 x 640`，竖屏。
- 当前前端存档：浏览器 `localStorage`；正式服权威数据：Rust/Axum + MySQL 服务端。
...
├─ Server/
│  ├─ src/             # Rust/Axum API、认证和业务处理
│  ├─ migrations/      # MySQL 数据库结构
...
| `config/AchievementConfig.ts` | 四类成就、进度类型、徽章和奖励 |
| `config/TitleConfig.ts` | 等级成长与成就限定称号、解锁条件和显示名 |
| `config/DailySignInConfig.ts` | 七日签到奖励配置 |
...
- 图片密集面板在主界面稳定后以小批次预热资源，避免打开面板的同一帧集中下载、解码图片。
...
- 服务端数据库统一使用 MySQL；客户端正式接入后，登录存档、货币与背包变更、制作队列、任务/成就/签到奖励必须通过接口确认，客户端不得自行生成最终结果。
...
- 当前规模不强制依赖 Redis。MySQL 足以承担玩家档案、存档和低并发排行榜；当出现多实例登录会话、排行榜热点缓存、广告计数限流、分布式锁或高频临时状态时再引入 Redis。
- 分享和排行榜依赖微信登录后的稳定玩家身份；广告能力按当前上线计划，在达到平台要求的 500 用户门槛后再接入客户端。
...
- 手机端浏览器适配已经过多种分辨率验证；平板电脑需要独立布局断点与专项测试。
- 背包与合成属于图片密集界面，继续优先使用分批预加载、可视区域渲染、节点复用和减少整页重建来降低首次打开卡顿。
